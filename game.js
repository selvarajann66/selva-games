...
function hud(){const jb=jetpackHit();txt("SCORE "+state.score,125,55,36,"left");txt("HI "+state.hi,state.w-130,55,36,"right");txt("🪙 "+state.coins,125,100,24,"left");txt("❤️ x"+state.hearts,state.w-130,100,24,"right");btn(20,15,90,40,"BACK");btn(state.w-110,15,90,40,state.jetMode?"JETPACK ON":state.dev?"DEV ON":"DEV");if(state.mode==="PLAYING")btn(state.w-110,65,90,40,"PAUSE");const sm=state.w<700,iy=sm?104:120,ih=sm?38:50,ic=sm?24:34,fs=sm?16:22;ctx.fillStyle="#141e3277";ctx.fillRect(20,iy,sm?245:340,ih);drawImg(asset("powerups","shield"),28,iy+(ih-ic)/2,ic,ic);txt("x"+state.shield,28+ic+5,iy+ih*.68,fs,"left");drawImg(asset("collectibles","gem"),110,iy+(ih-ic)/2,ic,ic);txt("x"+state.gems,110+ic+5,iy+ih*.68,fs,"left");drawImg(asset("powerups","rocket_powerup"),190,iy+(ih-ic)/2,ic,ic);txt(state.jetMode?"∞":"x"+state.jetpack,190+ic+5,iy+ih*.68,fs,"left");if(state.noteT>0)txt(state.note,state.w/2,220,34);if(state.mode==="READY")btn(20,120,130,50,"SHOP")}
...
window.addEventListener("DOMContentLoaded",function(){const c=document.querySelector("canvas");if(c){c.style.touchAction="none";c.style.pointerEvents="auto";["touchstart","touchmove","touchend"].forEach(function(t){c.addEventListener(t,function(e){if(e.cancelable)e.preventDefault()},{passive:false})})}});
