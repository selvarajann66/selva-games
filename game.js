...
function hud(){txt("SCORE "+state.score,125,55,36,"left");txt("HI "+state.hi,state.w-130,55,36,"right");txt("🪙 "+state.coins,125,100,24,"left");txt("❤️ x"+state.hearts,state.w-130,100,24,"right");btn(20,15,90,40,"BACK");btn(state.w-110,15,90,40,state.jetMode?"JETPACK ON":state.dev?"DEV ON":"DEV");if(state.mode==="PLAYING")btn(state.w-110,65,90,40,"PAUSE");ctx.fillStyle="#141e3277";ctx.fillRect(20,120,340,50);drawImg(asset("powerups","shield"),35,128,34,34);txt("x"+state.shield,74,153,22,"left");drawImg(asset("collectibles","gem"),145,128,34,34);txt("x"+state.gems,184,153,22,"left");drawImg(asset("powerups","rocket_powerup"),255,128,34,34);txt(state.jetMode?"∞":"x"+state.jetpack,294,153,22,"left");if(state.noteT>0)txt(state.note,state.w/2,220,34);if(state.mode==="READY")btn(20,120,130,50,"SHOP")}
function ready(){txt("DOODLE JUMP",state.w/2,state.h*.28,52);txt("SELECT DIFFICULTY & TAP TO JUMP",state.w/2,state.h*.36,28);["EASY","NORMAL","HARD","ULTRA_HARD"].forEach((k,i)=>btn(state.w*.16,state.h*.5+i*68,state.w*.68,54,DIFF[k].name,k===state.difficulty))}
...
function input(x,y,down){if(state.shop){if(!down)return;const pw=state.w*.9,ph=state.h*.8,sx=(state.w-pw)/2,sy=(state.h-ph)/2;if(x<sx||x>sx+pw||y<sy||y>sy+ph){state.shop=false;return}for(let i=0;i<4;i++){const yy=sy+180+i*145;if(hit(x,y,sx+pw-180,yy+25,125,70)){const prices=[100,150,200,250];if(state.coins>=prices[i]){state.coins-=prices[i];if(i===0)state.gems++;if(i===1)state.hearts++;if(i===2)state.shield++;if(i===3)state.jetpack++;save();note("PURCHASED");}return}}return}
 ...
 if(state.mode==="PLAYING"&&hit(x,y,230,112,125,72)){activateJetpack();return}
...
canvas.addEventListener("pointerdown",e=>{e.preventDefault();const r=canvas.getBoundingClientRect();const x=e.clientX-r.left,y=e.clientY-r.top;input(x,y,true);const onJetpackButton=state.mode==="PLAYING"&&hit(x,y,230,112,125,72);if(state.mode==="PLAYING"&&!onJetpackButton){if(x<state.w/2)state.left=true;else state.right=true}});
canvas.addEventListener("pointermove",e=>{if(state.mode==="PLAYING"&&e.buttons){const r=canvas.getBoundingClientRect(),x=e.clientX-r.left;state.left=x<state.w/2;state.right=!state.left}});
